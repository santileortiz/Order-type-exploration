...
void print_x11_property (xcb_connection_t *c, xcb_drawable_t window, xcb_atom_t property)
{
    ...
    if (type == get_x11_atom (c, "STRING") || type == get_x11_atom (c, "UTF8_STRING")) {
...
void send_selection_notify_event (xcb_connection_t *c, xcb_selection_request_event_t *ev)
{
    xcb_selection_notify_event_t rep;
    rep.response_type = XCB_SELECTION_NOTIFY;
    rep.time = ev->time;
    rep.requestor = ev->requestor;
    rep.selection = ev->selection;
    rep.target = ev->target;
    rep.property = ev->property;

    xcb_void_cookie_t ck = xcb_send_event_checked (c, 0, ev->requestor, 0, (const char*)&rep);
    xcb_generic_error_t *error; 
    if ((error = xcb_request_check(c, ck))) { 
        printf("Error sending selection notify event. %d\n", error->error_code); 
        free(error); 
    }
}
        ...
        while ((event = xcb_poll_for_event (x_st->xcb_c))) {
            ...
            switch (event->response_type & ~0x80) {
                case XCB_SELECTION_REQUEST:
                    ...
                    if (selection_request_ev->target == get_x11_atom (x_st->xcb_c, "TARGETS")) {
                        ...
                        send_selection_notify_event (x_st->xcb_c, selection_request_ev);

                    } else if (selection_request_ev->target == get_x11_atom (x_st->xcb_c, "TIMESTAMP")) {
                        xcb_change_property (x_st->xcb_c,
                                             XCB_PROP_MODE_REPLACE,
                                             selection_request_ev->requestor,
                                             selection_request_ev->property,
                    ...
                    } else if (selection_request_ev->target == get_x11_atom (x_st->xcb_c, "UTF8_STRING") ||
                               selection_request_ev->target == get_x11_atom (x_st->xcb_c, "TEXT") ||
                               selection_request_ev->target == get_x11_atom (x_st->xcb_c, "text/plain;charset=utf-8")) {
                        xcb_atom_t type;
                        if (type == get_x11_atom (x_st->xcb_c, "TEXT")) {
                        ...
                        xcb_change_property (x_st->xcb_c,
                                             ...
                                             type,
                                             8,
                                             strlen(st->gui_st.clipboard_str)+1,
